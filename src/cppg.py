...
from src.workflows.generate_reflection import GenReflectionWorkflow
...
class CPPG:
    ...
    async def generate_reflection(self, problem: dict) -> dict:
        config = settings.GenReflectionWorkflow
        result = await GenReflectionWorkflow(timeout=config.timeout, verbose=config.verbose).run(problem=problem)
        return result
